  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        } else if (res.ok && data.authenticated && data.role === "admin") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/admin/login", {
        ...
        credentials: "include",
        body: JSON.stringify({ email, password, passKey, role: "platform_manager" }),
      });
      ...
      if (response.ok && data.role === "platform_manager") {
        ...
        alert(data.message || "Invalid platform manager credentials");
      }
    } catch (error) {
